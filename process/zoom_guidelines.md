...
Zoom is the main video communication platform for ODPi. It is used for running the both techincal community and member meetings, along with many other ODPi online events. Since the Zoom meetings are open to the general public, a Zoom host or co-host has to moderate a meeting in all senses of the word from starting and stopping the meeting, to acting on [ODPi Code of Conduct] issues.

These guidelines are meant as a tool to help ODPi community members manage their Zoom resources.

## Code of conduct

The ODPi adheres to the [ODPi Code of Conduct] throughout all platforms and includes all communication mediums.

## Setting up your meeting and moderation

New Zoom meetings can be provisioned by contacting the [ODPi Program Manager]. Note that Zoom meetings are provisioned in a shared account across all ODPi activities, so you will need to coordinate use across other projects. A good way to do this is by reviewing the [ODPi calendar], which maintains a list of all community meetings.

ODPi Zoom meetings are provisioned with the following settings set to maximize security yet make community access to meetings have a fairly low barrier to entry:
...
- After an action has been taken, use the **lock meeting** feature so that no one else can come into the meeting. If that fails, end the call immediately, and contact the [ODPi Program Manager] to report the issue.
...
Hosts **must** be comfortable with how to use these moderation tools and the Zoom settings in general. Make sure whoever is running your meeting is equipped with the right knowledge and skills. If you have any questions or concerns, reach out to the [ODPi Program Manager] and they will be able to provide further guidance and training.
...
[ODPi Program Manager] should be escalated to the [ODPi TSC].
...
Contact [ODPi Program Manager] if you need help to edit a video before posting it to the public.
...
Thanks for making ODPi meetings work great!

[ODPi Code of Conduct]: ../CODE_OF_CONDUCT.md
[ODPi Program Manager]: mailto:pm@odpi.org
[ODPi calendar]: https://lists.odpi.org/calendar
[ODPi TSC]: mailto:odpi-tsc@lists.odpi.org
